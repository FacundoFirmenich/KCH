...
from .general_cleanup import GeneralCleanup
...
def parser() -> argparse.ArgumentParser:
    ...
    discover = commands.add_parser("discover-general")
    discover.add_argument("--home", type=Path)
    discover.add_argument("--adhoc-root", action="append", default=[], type=Path)
    discover.add_argument("--agent-root", action="append", default=[], type=Path)
    discover.add_argument("--temp-root", action="append", default=[], type=Path)
    discover.add_argument("--output", type=Path)

    plan = commands.add_parser("plan-general")
    plan.add_argument("discovery", type=Path)
    plan.add_argument("--older-than-hours", type=float, default=24.0)
    plan.add_argument("--active-path", action="append", default=[], type=Path)
    plan.add_argument("--replicated-receipt", type=Path)
    plan.add_argument("--output", type=Path)

    execute = commands.add_parser("execute-general")
    execute.add_argument("plan", type=Path)
    execute.add_argument("--actor", required=True, choices=["USER"])
...
def main() -> int:
    ...
    elif args.command == "discover-general":
        result = GeneralCleanup.discover(
            home=args.home,
            adhoc_roots=args.adhoc_root,
            agent_roots=args.agent_root,
            temp_roots=args.temp_root,
        )
    elif args.command == "plan-general":
        replicated = load_json(args.replicated_receipt) if args.replicated_receipt else None
        result = GeneralCleanup.plan(
            load_json(args.discovery),
            older_than_hours=args.older_than_hours,
            active_paths=args.active_path,
            replicated_receipt=replicated,
        )
    else:
        result = GeneralCleanup.execute(
            load_json(args.plan),
            actor=args.actor,
            exact_authorization_id=args.authorization_id,
