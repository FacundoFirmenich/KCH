...
class PathPolicy:
    ...
    def writable_directory(self, path: str | Path) -> Path:
        ...
        if not self._inside(resolved, self.write_roots):
...
class MISFullCSIBridge:
    ...
    def __init__(self, wheel: str | Path, *, path_policy: PathPolicy | None = None) -> None:
        ...
        if self.surface["unbound_exports"]:
            raise MISBridgeError(f"unbound MIS __all__ exports: {self.surface['unbound_exports']}")
    ...
    def _status(self, _arguments: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "declared_exports": len(self.surface["declared_exports"]),
            "unbound_exports": self.surface["unbound_exports"],
    ...
    def _bayes_decide(self, a: dict[str, Any]) -> Any:
        decision = self.decision.bayes_decide(
            self._distribution(a["posterior"]),
            self._loss_table(a["loss_table"]),
            tie_action=a.get("tie_action"),
        )
        if decision.tie_resolution == "EXPLICIT_TIE_ACTION" and decision.resolved_action not in decision.minimizers:
            raise ValueError("tie_action must select one of the Bayes-risk minimizers")
