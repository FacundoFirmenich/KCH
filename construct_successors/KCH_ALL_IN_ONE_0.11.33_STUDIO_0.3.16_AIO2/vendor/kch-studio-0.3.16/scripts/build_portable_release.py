...
KCH_RELEASE_VERSION = "0.11.33"
KCH_RELEASE_LINE = "R33"
PACKAGE_NAME = f"KCH_{KCH_RELEASE_VERSION}_PRE2G_{KCH_RELEASE_LINE}"
ARCHIVE_NAME = f"KCH_{KCH_RELEASE_VERSION}_PRE2G_INTEGRATED_CANDIDATE_{KCH_RELEASE_LINE}.zip"
...
def main() -> None:
    ...
    base = Path(
        os.environ.get(
            "KCH_FROZEN_BASE_ROOT", str(workspace / "KCH_0.11_REEXTRACT_FINAL")
        )
    ).resolve()
    base_wheelhouse = Path(
        os.environ.get("KCH_FROZEN_BASE_WHEELHOUSE", str(base / "dist"))
    ).resolve()
    mis = Path(
        os.environ.get(
            "KCH_MIS_ROOT_SOURCE", str(workspace / "KCH_MIS03_REEXTRACT_v0.1.0")
        )
    ).resolve()
    governance = candidate / "governance"
    for label, required in (
        ("frozen base source", base),
        ("frozen base wheelhouse", base_wheelhouse),
        ("MIS source", mis),
        ("governance source", governance),
    ):
        if not required.is_dir():
            raise FileNotFoundError(f"{label} is unavailable: {required}")
    base_core_wheels = sorted(base_wheelhouse.glob("kwancode_harness-0.11.0-*.whl"))
    if len(base_core_wheels) != 1:
        raise RuntimeError(
            "expected exactly one sealed kwancode_harness 0.11.0 wheel, "
            f"found {len(base_core_wheels)} in {base_wheelhouse}"
        )
    final_output = Path(
    ...
    if dependency_wheels.is_dir():
        for source in dependency_wheels.glob("*.whl"):
            shutil.copy2(source, wheelhouse / source.name)
    required_portable_wheel_prefixes = (
        "kch_csi_studio_extension_fabric-",
        "kwancode_harness-",
        "numpy-",
        "scipy-",
        "pillow-",
    ...
    wheel_names = {path.name.casefold() for path in wheelhouse.glob("*.whl")}
    missing_wheels = [
        prefix
        for prefix in required_portable_wheel_prefixes
        if not any(name.startswith(prefix) for name in wheel_names)
    ]
    if missing_wheels:
        raise RuntimeError(
            f"portable Windows wheelhouse is incomplete: {missing_wheels}"
        )
    ...
    shutil.copy2(
        ...
        final_output / f"EXTRACT_AND_INSTALL_KCH_{KCH_RELEASE_LINE}.py",
    )
    write(
        final_output / f"EXTRACT_AND_INSTALL_KCH_{KCH_RELEASE_LINE}.cmd",
        '@echo off\r\npython "%~dp0EXTRACT_AND_INSTALL_KCH_R33.py" "%~dp0KCH_0.11.33_PRE2G_INTEGRATED_CANDIDATE_R33.zip"\r\n',
    )
    print(
        json.dumps(
            {
                ...
                "max_archive_member_characters": max_member_characters,
                "safe_short_extractor": str(final_output / f"EXTRACT_AND_INSTALL_KCH_{KCH_RELEASE_LINE}.cmd"),
            },
            ensure_ascii=False,
            indent=2,
        )
