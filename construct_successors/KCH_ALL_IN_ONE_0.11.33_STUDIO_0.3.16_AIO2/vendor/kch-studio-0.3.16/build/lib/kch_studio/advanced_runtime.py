...
from kch_instruction_governance import KCHInstructionGovernance
...
from .libresource import LibresourceRuntime
...
class KCHAdvancedRuntime:
    def __init__(
        ...
        *,
        ...
        self.instruction_governance = KCHInstructionGovernance(
            self.root / "instruction_governance"
        )
        self.libresource = LibresourceRuntime(self.root / "libresource")
        ...
        instruction_handlers = self.instruction_governance.handlers()
        instruction_overlap = set(self.handlers) & set(instruction_handlers)
        if instruction_overlap:
            raise ValueError(
                "duplicate instruction-governance handlers: "
                f"{sorted(instruction_overlap)}"
            )
        self.handlers.update(instruction_handlers)
        libresource_handlers = self.libresource.handlers()
        libresource_overlap = set(self.handlers) & set(libresource_handlers)
        if libresource_overlap:
            raise ValueError(
                "duplicate LIBRESOURCE handlers: " f"{sorted(libresource_overlap)}"
            )
        self.handlers.update(libresource_handlers)
        if extra_handlers:
        ...
        tool_by_name = {
            ...
            for item in [
                *ADVANCED_TOOLS,
                *self.instruction_governance.tool_descriptors(),
                *self.libresource.tool_descriptors(),
                *(extra_tools or []),
            ]
        }
    ...
    def status(self) -> dict[str, Any]:
        components = {
            ...
            "instruction_governance": self.instruction_governance.status(),
            "libresource": self.libresource.status(),
