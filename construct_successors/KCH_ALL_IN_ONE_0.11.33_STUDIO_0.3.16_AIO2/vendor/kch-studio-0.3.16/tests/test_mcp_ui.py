...
from kch_studio.mcp_server import PROTOCOL_VERSION, TOOLS, StudioMCP
...
def test_super_mcp_exposes_standard_resources_without_msp(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("KCH_011_HMAC_SECRET", "0123456789abcdef0123456789abcdef")
    monkeypatch.setenv("KCH_011_STATE", str(tmp_path / "resources-kch-011.sqlite3"))
    server = IntegratedSuperMCP(tmp_path / "resources-integrated")
    try:
        initialized = server.handle(
            {"jsonrpc": "2.0", "id": 1, "method": "initialize", "params": {}}
        )
        assert initialized["result"]["protocolVersion"] == PROTOCOL_VERSION
        assert initialized["result"]["serverInfo"]["name"] == "kch-super-mcp"
        ...
        status = json.loads(status_response["result"]["contents"][0]["text"])
        assert status["status"] == "READY_BOUNDED"
        assert status["storagePolicy"]["authoritativeStore"] == "GOOGLE_DRIVE"
        assert status["authorityInherited"] is False
        assert status["automaticPromotion"] is False
        assert server.studio.advanced is None
        ...
        unknown = server.handle(
            {
                ...
                "id": 4,
                "method": "resources/read",
                "params": {"uri": "kch://super-mcp/unknown"},
            }
        )
        assert unknown["error"]["code"] == -32602
        assert server.handle({"jsonrpc": "2.0", "id": 5, "method": "ping"})["result"] == {}
    finally:
        server.close()
...
def test_frozen_kch_super_mcp_overlay_composition(tmp_path: Path, monkeypatch) -> None:
    ...
    assert receipt["storage_policy"]["authoritativeStore"] == "GOOGLE_DRIVE"
