...
## Llaves constitucionales opcionales

Las llaves vienen desactivadas y sólo el usuario puede activarlas, crearlas, desactivarlas o autorizar una excepción desde una interfaz local confiable. Si una herramienta devuelve `BLOCKED_EXACT_USER_AUTHORIZATION_REQUIRED`, no repitas, no reduzcas ni reformules silenciosamente la mutación. Usa `lock_tool_call_propose` o `lock_change_propose` para explicar razón, impacto, dependencias y recuperación, manteniendo exactamente recurso, operación, preimagen, argumentos y resultado pretendido. Espera la autorización local exacta y ejecútala una sola vez mediante `lock_authorized_execute`.

No existe autorización de bloqueo para toda la sesión. `ALWAYS_THIS_SESSION`, permisos generales, automatización, RUN y CONSTRUCT no desbloquean una llave. Ninguna herramienta MCP permite al modelo crear o autorizar llaves. La prevención cubre sólo mutaciones mediadas por KCH; usa `lock_drift_verify` para detectar cambios externos en archivos exactos con línea base.
...
## Clausura mecánica sin degradación de evidencia

Cuando una tarea combine lectura completa y supervisión terminal, entrega sin compactar el `full_read_batch` original, su `full_read_verification` y el `monitor_evidence` a `evidence_closure_materialize`. Persiste la clausura devuelta sin reconstruirla, resumir sus recibos ni volver a transcribir hashes, líneas, spans, PID o código de salida. Antes del mensaje final pásala a `evidence_closure_verify`; sólo `PASS_VERIFIED` permite usar la proyección como recibo final. Un resumen humano puede referenciar la clausura, pero nunca sustituirla ni reparar manualmente una discrepancia.
